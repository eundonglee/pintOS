               ...
               thread_func *function, void *aux) 
{
  ...
  /* If priority of the new thread is higher than priority of the current thread, yield the CPU to the new thread. */
  test_max_priority ();
...
thread_unblock (struct thread *t) 
{
  ...
  /* Push unblocked thread into ready_list ordered by priority. */
...
thread_yield (void) 
{
  ...
  old_level = intr_disable ();
  if (cur != idle_thread) 
	/* Push current thread into ready_list ordered by priority. */
...
/* If the highest priority of the threads in ready_list if higher than priority of the current thread, yield the CPU to the top priority thread. */
void test_max_priority (void)
{
  struct thread *cur;
  struct thread *first_t;
  struct list_elem *first_e;
  enum intr_level old_level;
  ...
  {
    cur = thread_current ();
    first_e = list_begin (& ready_list);
    first_t = list_entry (first_e, struct thread, elem);
    if (first_t -> priority > cur -> priority)
	{
	  list_insert_ordered (& ready_list, & cur->elem, cmp_priority, NULL);
	  cur -> status = THREAD_READY;
	  schedule ();
...
/* If priority of the first element is higher than the second, return true. Otherwise, return false. */
...
thread_get_load_avg (void) 
{
  ...
  load_avg_sec = fp_to_int_round (mult_mixed (load_avg, 100));
...
thread_get_recent_cpu (void) 
{
  ...
  recent_cpu_sec = fp_to_int_round (mult_mixed (thread_current ()->recent_cpu, 100));

  intr_set_level (old_level);

  return recent_cpu_sec;
}

/* Refresh priority of the thread for mlfqs. */
void mlfqs_priority (struct thread *t)
{
  if (t != idle_thread)
  {
	t->priority = PRI_MAX - fp_to_int_round (div_mixed (t->recent_cpu, 4)) 
				  - (t->nice * 2);
  }
}

/* Refresh recent_cpu of the thread for mlfqs. */
...
void mlfqs_load_avg (void)
{
  ...
  /* Count running and ready threads except for idle thread. */
  ready = thread_current () != idle_thread ? 1 : 0;
  ...
  temp = add_fp (div_mixed (mult_mixed (load_avg, 59), 60),
				 div_mixed (int_to_fp (ready), 60));

  load_avg = temp >= 0 ? temp : 0;
}

/* Increments recent_cpu of the current thread. */
...
/* Recalculate priority and recent_cpu of the all threads. */
